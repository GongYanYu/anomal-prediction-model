...
window_size = 3 # sequence window length
# embed_dim = 300
num_classes = 142
input_size = 1 # input vector length
hidden_size = 64 # sequence embedding size
num_layers = 1
num_epochs = 50
batch_size = 2048
attention_size = 16
...
def generate(file_path):
    ...
    with open(file_path, 'r',encoding='utf-8') as f:
        for line in tqdm(f.readlines(),desc='read dataset'):
            num_sessions += 1
            # line = tuple(map(lambda n: n - 1, map(int, line.strip() .split())))
            line = tuple(map(lambda n: n, map(int, line.strip().split())))
            for i in range(len(line) - window_size):
    ...
    print('Number of sessions({}): {}'.format(file_path, num_sessions))
    print('Number of seqs({}): {}'.format(file_path, len(inputs)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-embeddim', default=1, type=int)
    parser.add_argument('-timedim', default=1, type=int)
...
#    import pdb; pdb.set_trace()
   ...
   #     tensortype = torch.long
    model = BiLSTM(batch_size, embed_dim, hidden_size, num_layers, num_classes,  bidirectional,
                   True, attnFlag, time_dim, device,0).to(device)
    #model = AC_BiLSTM(embed_dim, hidden_size, num
    # _layers, num_classes, bidirectional, device).to(device)
    seq_dataset = generate(file_path='../../DeepLog0806/data/spark/normal-spark-test')
    dataloader = DataLoader(seq_dataset, batch_size=batch_size, shuffle=True, pin_memory=True)


    # Loss and optimizer 交叉熵损失函数
   ...
   # import pdb; pdb.set_trace()
    for epoch in range(num_epochs):
        ...
        for step, (seq, label) in enumerate(dataloader):
            #import pdb; pdb.set_trace()
            y, output, seq_len = model(seq.to(device), label.to(device))
